...
<div class="space-y-6">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Receitas</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">Gerencie suas entradas financeiras</p>
        </div>
        <button onclick="openModal()"
            class="w-full md:w-auto px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg flex items-center justify-center gap-2 transition shadow-md">
...
<div id="modal-form"
    ...
    <div class="bg-white dark:bg-gray-800 w-full max-w-md rounded-2xl p-6 shadow-2xl transform transition-all">
        <h3 class="text-xl font-bold dark:text-white mb-4" id="modal-title">Nova Receita</h3>
...
<script>
    const API_LIST = 'api/list_receitas.php';
    const API_CREATE = 'api/create_receita.php';
    const API_UPDATE = 'api/update_receita.php';
    const API_DELETE = 'api/delete_receita.php';
    ...
    function render() {
        ...
        if (currentItems.length === 0) {
            mobileList.innerHTML = '<div class="text-center text-gray-500 py-8">Nenhuma receita encontrada.</div>';
            return;
        }
        ...
        currentItems.forEach(item => {
            ...
            tr.innerHTML = `
                ...
                <td class="p-4 text-sm"><span class="px-2 py-1 bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300 rounded text-xs font-semibold">${item.categoria}</span></td>
                <td class="p-4 text-sm font-bold text-green-600 dark:text-green-400">${valorFmt}</td>
            ...
            card.innerHTML = `
                <div>
                    ...
                    <div class="flex items-center gap-2 text-xs text-gray-500 mt-1">
                        <span>${dataFmt}</span>
                        <span>•</span>
                        <span class="text-green-600 dark:text-green-400">${item.categoria}</span>
                    </div>
                </div>
                <div class="text-right">
                    <p class="font-bold text-green-600 dark:text-green-400">${valorFmt}</p>
    ...
    window.editItem = function (item) {
        ...
        document.getElementById('rec-data').value = item.data; // YYYY-MM-DD
    ...
    // Auto load if query params exist (e.g. from dashboard shortcut)
