...
// Verifica ownership
$check = $db->prepare("SELECT id FROM receitas WHERE id=:id AND user_id=:user_id");
...
$query = "UPDATE receitas SET descricao=:descricao, valor=:valor, categoria=:categoria, data=:data WHERE id=:id";
...
if ($stmt->execute()) {
    sendJSON(["message" => "Receita atualizada."]);
} else {
