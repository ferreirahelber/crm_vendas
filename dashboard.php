...
<div class="space-y-6 animate-fade-in-up">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Olá, <span
                    id="user-name"><?php echo htmlspecialchars($_SESSION['nome']); ?></span> 👋</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">Aqui está o resumo financeiro.</p>
        </div>
        <div id="status-badge"
            class="hidden px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide shadow-sm">
            <!-- Preenchido via JS -->
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div
                class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
                <h3 class="text-lg font-semibold text-gray-800 dark:text-white mb-4">Últimos Lançamentos</h3>
                ...
                <div class="mt-4 text-center">
                    <a href="receitas.php" class="text-sm text-indigo-600 dark:text-indigo-400 hover:underline">Ver
                        tudo</a>
                </div>
            </div>
        </div>
            ...
            try {
                ...
                if (data.recentes.length === 0) {
                    ...
                    data.recentes.forEach(item => {
                        ...
                        el.className = 'flex items-center justify-between p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition';
                        el.innerHTML = `
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <p class="font-medium text-gray-800 dark:text-gray-200">${item.descricao}</p>
                                <p class="text-xs text-gray-500 capitalize">${item.categoria} • ${dateDesc}</p>
                            </div>
                        </div>
                        <span class="font-semibold ${isReceita ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}">
                ...
                new Chart(ctx, {
                    ...
                    data: {
                        ...
                        datasets: [
                            {
                                label: 'Receitas',
                                data: data.chart.receitas,
                                backgroundColor: '#10B981',
                                borderRadius: 4
                            },
                            {
                                label: 'Despesas',
                                data: data.chart.despesas,
                    ...
                    options: {
                        ...
                        plugins: {
                            legend: { position: 'bottom', labels: { color: document.documentElement.className.includes('dark') ? '#ccc' : '#666' } }
                        },
                        scales: {
                            y: {
                                beginAtZero: true,
                                grid: { color: document.documentElement.className.includes('dark') ? '#374151' : '#e5e7eb' },
                                ticks: { color: document.documentElement.className.includes('dark') ? '#9ca3af' : '#4b5563' }
                            },
                            x: {
                                grid: { display: false },
                                ticks: { color: document.documentElement.className.includes('dark') ? '#9ca3af' : '#4b5563' }
